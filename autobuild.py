...
def package_python_program():
    ...
    if args.releases:
        nuitka_options = f"--standalone --include-package=ui --include-package=language --windows-icon-from-ico=ico.ico --show-memory --show-progress --nofollow-imports --plugin-enable=pyside6 --output-dir=build/releases/ --windows-file-version={args.version_code} --mingw64 --follow-import-to=src" + " " +args.other
        output_file = os.path.join("build","releases","decksmanager.dist")
    else:
        nuitka_options = f"--standalone --include-package=ui --include-package=language --windows-icon-from-ico=ico.ico --show-memory --show-progress --nofollow-imports --plugin-enable=pyside6 --output-dir=build\{args.version_code} --windows-file-version={args.version_code} --mingw64 --follow-import-to=src" + " " + args.other
